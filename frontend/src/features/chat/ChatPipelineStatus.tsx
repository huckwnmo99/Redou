import { Search, BrainCircuit, Table2, ShieldCheck, Check, Code } from "lucide-react";
...
export function ChatPipelineStatus({ stage, message }: Props) {
  ...
  // "searching" onwards = full stepper for table generation pipeline
  const activeIdx = tableStageIndex(stage);
  ...
  return (
    <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
      ...
      >
        {TABLE_STAGES.map((s, idx) => {
          const isDone = idx < activeIdx;
          const isActive = idx === activeIdx;
          const isPending = idx > activeIdx;
          const Icon = s.icon;
          const isLast = idx === TABLE_STAGES.length - 1;
