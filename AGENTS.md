...
1. After Claude reviews the first Phase 1C abort/error tracer, continue Phase 1C with the next error-path slice: either the non-abort per-paper extraction error fallback scenario from the fake catalog or one minimal import/embedding worker failure path.
