...
- Test foundation Phase 1A is drafted as a docs/harness slice: `docs/features/fix/13-test-foundation-phase1a.md` defines the fixture strategy and harness skeleton, ADR 0006 proposes isolated local Supabase as the primary integration target with deterministic service fakes, `docs/harness/fixtures/` defines the planned golden-path corpus, and Claude review is requested before Phase 1B runtime tests.
- Claude reviewed Phase 1A with GO and no blocker; the P2 correction is applied so embedding fixtures now require 2048-dim vectors, ADR 0006 is accepted, Phase 1B schema provenance is defined as full repository migration-stream setup in a disposable test target, and the first golden-path contract is narrowed to the core paper-to-table persistence spine.
- Phase 1B first tracer is implemented as an opt-in desktop Node integration test: `apps/desktop/tests/integration/golden-path.test.mjs` refuses the normal Redou dev Supabase target, skips the real DB spine unless disposable local Supabase env vars are provided, and when configured seeds a tiny paper/chunk/2048-dim embedding/table fixture before exercising real `createMultiQueryRag(...)` RPC retrieval and `runTableConversationPipeline(...)` table persistence with deterministic fakes.
- Phase 1B disposable Supabase runner is implemented: `apps/desktop` now has `npm run test:integration:supabase`, which creates a temp Supabase project on non-dev ports, copies repository migrations, disables seed/external Google OAuth for the target, starts only the needed local services including Auth, resets the disposable DB, runs the golden-path integration test against the real REST/RPC surface, then stops the target. This first real run surfaced and fixed a stale 4-argument `match_chunks` overload via `supabase/migrations/20260524010000_drop_stale_match_chunks_overload.sql`, and desktop RAG now calls the 6-argument RPC shape explicitly.
- Phase 1C first abort/error tracer is implemented: the golden-path fixture now has a small fake-service catalog with happy-path, per-paper abort, and per-paper error scenarios; deterministic service helpers can instantiate the abort fake; the real disposable Supabase integration now verifies that aborting during per-paper extraction leaves no `chat_messages` or `chat_generated_tables` rows and preserves the seeded conversation phase.
- Phase 1C per-paper error fallback tracer is implemented: the catalogued `perPaperError` fake now has a deterministic single-call fallback table generator, default Node coverage verifies the fake scenario, and the real disposable Supabase integration verifies `single_call_fallback` persistence, partial-failure metadata, skipped agentic-recovery metadata, one `table_report` message, and `follow_up` conversation phase.
...
- `apps/desktop`: `node --check electron\processing\job-runner.mjs`, `node --check electron\main.mjs`, `node --check tests\integration\golden-path.test.mjs`, and `node --check tests\processing-job-runner.test.mjs` pass after the minimal worker failure tracer.
- `apps/desktop`: `node --test tests\processing-job-runner.test.mjs tests\integration\*.test.mjs` passes with approved escalation; current focused safety-mode result is 2 suites, 6 tests, 4 expected skipped DB tests.
- `apps/desktop`: `cmd /c npm run test:integration:supabase` passes after the minimal worker failure tracer; current disposable real DB result is 1 suite, 5 tests, 0 skipped.
- `apps/desktop`: `cmd /c npm run test` passes after the minimal worker failure tracer; current desktop default result is 10 suites, 49 tests.
- `apps/desktop`: `cmd /c npm run build` passes after the minimal worker failure tracer.
- `git diff --check` passes after the minimal worker failure tracer with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the touched docs, processing helper, main worker wiring, and tests.
- `git diff --check` passes after the Phase 2A eval schema docs with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the touched docs.
- `apps/desktop`: Phase 2B runnable eval RED was observed with approved escalation: `node --test tests\eval-runner.test.mjs` failed on missing `tests\integration\support\eval-runner.mjs`.
- `apps/desktop`: `node --check tests\integration\support\eval-runner.mjs`, `node --check tests\eval-runner.test.mjs`, and `node --check tests\integration\golden-path.test.mjs` pass after the runnable eval tracer.
- `apps/desktop`: `node --test tests\eval-runner.test.mjs` passes with approved escalation; current eval helper result is 1 suite, 4 tests.
- `apps/desktop`: `node --test tests\eval-runner.test.mjs tests\integration\*.test.mjs` passes with approved escalation in safety mode; current focused result is 2 suites, 10 tests, 5 expected skipped DB tests.
- `apps/desktop`: `cmd /c npm run test:integration:supabase` passes after the runnable eval tracer; current disposable real DB result is 1 suite, 6 tests, 0 skipped.
- `apps/desktop`: `cmd /c npm run test` passes after the runnable eval tracer; current desktop default result is 11 suites, 53 tests.
- `apps/desktop`: `cmd /c npm run build` passes after the runnable eval tracer.
- `git diff --check` passes after the runnable eval tracer with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in touched docs/tests/fixtures.

### Verified 2026-05-24
- `apps/desktop`: Phase 1B runner RED was observed after rerunning the Node test runner with approved escalation: `tests\integration-runner.test.mjs` failed on missing `scripts/run-golden-path-supabase.mjs`.
- `apps/desktop`: `node --check scripts\run-golden-path-supabase.mjs` and `node --check electron\rag\multi-query-rag.mjs` pass after the disposable runner and explicit `match_chunks` RPC call-shape fixes.
- `apps/desktop`: `node --test tests\integration-runner.test.mjs tests\multi-query-rag.test.mjs` passes with approved escalation; current focused result is 2 suites, 7 tests.
- `apps/desktop`: `node --test tests\integration\*.test.mjs` passes in default safety mode with 1 dev-target refusal guard and 1 explicit skipped DB test when no disposable env is set.
- `apps/desktop`: `cmd /c npm run test:integration:supabase` passes against a disposable Supabase target on ports `55420-55429`; the real DB golden-path result is 1 suite, 2 tests, 0 skipped, and it applies every repository migration including `20260524010000_drop_stale_match_chunks_overload.sql`.
- `apps/desktop`: `cmd /c npm run test` passes after the disposable runner and RAG RPC/schema fix; current desktop default result is 8 suites, 45 tests.
- `apps/desktop`: `cmd /c npm run build` passes after the disposable runner and RAG RPC/schema fix.
- `git diff --check` passes after the disposable runner and RAG RPC/schema fix with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the touched docs, runner, tests, RAG module, and migration.
- `apps/desktop`: Phase 1C fake-catalog RED was observed after rerunning the Node test runner with approved escalation: `tests\deterministic-services.test.mjs` failed because `fixture.serviceCatalog` was missing and the `perPaperAbort` fake did not abort.
- `apps/desktop`: `node --check tests\integration\golden-path.test.mjs` and `node --check tests\integration\support\deterministic-services.mjs` pass after the Phase 1C abort/error tracer.
- `apps/desktop`: `node --test tests\deterministic-services.test.mjs tests\integration\*.test.mjs` passes with approved escalation; current focused safety-mode result is 2 suites, 5 tests, 2 skipped DB tests.
- `apps/desktop`: `cmd /c npm run test:integration:supabase` passes after the Phase 1C abort/error tracer; current disposable real DB result is 1 suite, 3 tests, 0 skipped.
- `apps/desktop`: `cmd /c npm run test` passes after the Phase 1C abort/error tracer; current desktop default result is 9 suites, 47 tests.
- `apps/desktop`: `cmd /c npm run build` passes after the Phase 1C abort/error tracer.
- `git diff --check` passes after the Phase 1C abort/error tracer with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the touched docs, integration test, deterministic service helper, fake-catalog test, and fake catalog JSON.
- `apps/desktop`: Phase 1C per-paper error fallback RED was observed in the disposable Supabase runner: the new test failed with `TypeError: runTableConversationPipeline requires generateTableFromSpecFn for Stage 3c fallback`.
- `apps/desktop`: `node --check tests\integration\golden-path.test.mjs` and `node --check tests\integration\support\deterministic-services.mjs` pass after the per-paper error fallback tracer.
- `apps/desktop`: `node --test tests\deterministic-services.test.mjs tests\integration\*.test.mjs` passes with approved escalation after sandbox `spawn EPERM`; current focused safety-mode result is 2 suites, 7 tests, 3 expected skipped DB tests.
- `apps/desktop`: `cmd /c npm run test:integration:supabase` passes after the per-paper error fallback tracer; current disposable real DB result is 1 suite, 4 tests, 0 skipped.
- `apps/desktop`: `cmd /c npm run test` passes after the per-paper error fallback tracer; current desktop default result is 9 suites, 48 tests.
- `apps/desktop`: `cmd /c npm run build` passes after the per-paper error fallback tracer.
- `git diff --check` passes after the per-paper error fallback tracer with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the touched docs, integration test, deterministic service helper, and fake-catalog test.

### Verified 2026-05-23
- `apps/desktop`: Phase 1B RED was observed after rerunning the Node test runner with approved escalation: `tests\integration\golden-path.test.mjs` failed on missing `support/supabase-test-target.mjs`.
- `apps/desktop`: `node --check tests\integration\golden-path.test.mjs`, `node --check tests\integration\support\supabase-test-target.mjs`, and `node --check tests\integration\support\deterministic-services.mjs` pass after the Phase 1B integration tracer.
- `apps/desktop`: `cmd /c npm run test:integration` passes in safety mode after the Phase 1B integration tracer; current result is 1 passing dev-target refusal guard and 1 skipped real DB core-spine test because no disposable Supabase target env is configured.
- `apps/desktop`: `cmd /c npm run test` passes after the Phase 1B integration tracer; current desktop default result is 7 suites, 43 tests.
- `apps/desktop`: `cmd /c npm run build` passes after the Phase 1B integration tracer.
- `git diff --check -- AGENTS.md docs\agents\codex-claude\codex-to-claude.md docs\features\fix\13-test-foundation-phase1a.md docs\harness\fixtures\README.md docs\harness\fixtures\golden-path\README.md apps\desktop\package.json apps\desktop\tests\integration\golden-path.test.mjs apps\desktop\tests\integration\support\supabase-test-target.mjs apps\desktop\tests\integration\support\deterministic-services.mjs` passes after the Phase 1B integration tracer with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the new runtime fixture/test docs and modules.
...
- `git diff --check -- AGENTS.md docs\agents\codex-claude\codex-to-claude.md` passes after the Phase 1A test-foundation docs with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the new Phase 1A docs.
- `git diff --check -- AGENTS.md docs\agents\codex-claude\codex-to-claude.md` passes after Claude Phase 1A review corrections with LF-to-CRLF warnings only on existing mixed-line-ending files, and `Select-String -Pattern "\s+$"` finds no trailing whitespace in the new Phase 1A docs.
...
| DONE | 2026-05-24 | Codex | Phase 1C per-paper error fallback integration tracer | `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/deterministic-services.mjs`, `apps/desktop/tests/deterministic-services.test.mjs`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` | Real external service calls, browser UI/Electron app launch, normal dev DB mutation/reset, import/embedding worker failure coverage | Claude Phase 1C abort/error tracer review GO and recommendation to exercise `perPaperError` fallback before import/worker failure paths |
| DONE | 2026-05-24 | Codex | Phase 1C abort/error integration tracer and fake catalog | `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/deterministic-services.mjs`, `apps/desktop/tests/deterministic-services.test.mjs`, `apps/desktop/tests/fixtures/golden-path/fakes/service-catalog.json`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` | Real external service calls, browser UI/Electron app launch, normal dev DB mutation/reset, broad import worker coverage, happy-path assertion expansion | Claude Phase 1B disposable runner review GO and recommendation to move to Phase 1C abort/error coverage plus fake catalog |
| DONE | 2026-05-24 | Codex | Phase 1B disposable Supabase target runner | `apps/desktop/package.json`, `apps/desktop/scripts/run-golden-path-supabase.mjs`, `apps/desktop/tests/integration-runner.test.mjs`, `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/supabase-test-target.mjs`, `apps/desktop/electron/rag/multi-query-rag.mjs`, `apps/desktop/tests/multi-query-rag.test.mjs`, `supabase/migrations/20260524010000_drop_stale_match_chunks_overload.sql`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` | Real external service calls, browser UI/Electron app launch, normal dev DB mutation/reset, entity graph implementation, import worker coverage | Claude Phase 1B review GO with next-slice requirement to execute the real DB branch on a disposable target |
| DONE | 2026-05-23 | Codex | Phase 1B golden-path integration test tracer | `apps/desktop/package.json`, `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/supabase-test-target.mjs`, `apps/desktop/tests/integration/support/deterministic-services.mjs`, `apps/desktop/tests/fixtures/golden-path/**`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/harness/fixtures/README.md`, `docs/harness/fixtures/golden-path/README.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` | Real external service calls, browser UI/Electron app launch, dev DB reset/mutation, entity graph implementation, Plan 12 Stage 5 refactor | User approval to proceed after Claude Phase 1A GO and corrections |
| DONE | 2026-05-22 | Codex | Phase 1A Claude review corrections | `docs/features/fix/13-test-foundation-phase1a.md`, `docs/harness/decisions/0006-test-fixture-strategy.md`, `docs/harness/fixtures/golden-path/README.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` | Phase 1B runtime test implementation, DB reset/migration execution, external service calls, entity graph implementation | Claude Phase 1A review GO with P2 embedding dimension correction and schema provenance clarification |
| DONE | 2026-05-22 | Codex | Phase 1A test fixture strategy and harness skeleton | `docs/features/fix/13-test-foundation-phase1a.md`, `docs/harness/decisions/0006-test-fixture-strategy.md`, `docs/harness/fixtures/README.md`, `docs/harness/fixtures/golden-path/README.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` | Runtime test implementation, DB migration/reset execution, entity graph implementation, Plan 12 Stage 5 runtime refactor, real external service calls | D35 closes Plan 12 and sets default next direction to test-foundation roadmap |
...
| 2026-05-24 | Codex | Implemented the Phase 1C per-paper error fallback tracer: added a disposable Supabase integration assertion for the catalogued `perPaperError` scenario, extended deterministic fake services with a single-call fallback table generator, verified fallback persistence with `single_call_fallback` metadata, `partialFailures`, skipped agentic-recovery metadata, one `table_report` message, and `follow_up` conversation phase, added default Node coverage for the fake scenario, and sent Claude a blocker/P1/P2 review request; verified RED, syntax checks, focused safety-mode tests, disposable Supabase integration, desktop tests, and desktop build | `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/deterministic-services.mjs`, `apps/desktop/tests/deterministic-services.test.mjs`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` |
| 2026-05-24 | Codex | Implemented the first Phase 1C abort/error tracer: added a golden-path fake service catalog with happy-path, per-paper abort, and per-paper error scenarios; extended deterministic services to instantiate catalogued scenarios; added default Node coverage for the catalog and parent-signal abort fake; added a disposable Supabase integration assertion that aborting during per-paper extraction persists no chat message or generated table and leaves the seeded conversation phase unchanged; verified fake-catalog RED, syntax checks, focused tests, disposable Supabase integration, desktop tests, and desktop build | `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/deterministic-services.mjs`, `apps/desktop/tests/deterministic-services.test.mjs`, `apps/desktop/tests/fixtures/golden-path/fakes/service-catalog.json`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` |
| 2026-05-24 | Codex | Implemented the Phase 1B disposable Supabase target runner: added `npm run test:integration:supabase`, generated an isolated temp Supabase project on non-dev ports from the repository migration stream, kept Auth enabled so service-role credentials are available, disabled target Google OAuth env references, made the integration test skip branch explicit, executed the real golden-path DB spine, and fixed the stale `match_chunks` overload discovered by the first real run by adding a cleanup migration and explicit 6-argument RAG RPC calls; verified runner RED, focused tests, integration safety mode, disposable Supabase integration, desktop tests, and desktop build | `apps/desktop/package.json`, `apps/desktop/scripts/run-golden-path-supabase.mjs`, `apps/desktop/tests/integration-runner.test.mjs`, `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/supabase-test-target.mjs`, `apps/desktop/electron/rag/multi-query-rag.mjs`, `apps/desktop/tests/multi-query-rag.test.mjs`, `supabase/migrations/20260524010000_drop_stale_match_chunks_overload.sql`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` |
| 2026-05-23 | Codex | Implemented the Phase 1B golden-path integration tracer: added `npm run test:integration`, a guarded opt-in desktop Node integration test, Supabase target safety/provenance support, deterministic 2048-dim embedding and LLM fixture services, runtime golden-path fixture JSON, and docs/Claude handoff updates; verified the RED missing-support failure, new file syntax checks, integration safety-mode pass, default desktop tests, and desktop build, while leaving the real DB core-spine branch pending until a disposable Supabase test target is configured | `apps/desktop/package.json`, `apps/desktop/tests/integration/golden-path.test.mjs`, `apps/desktop/tests/integration/support/supabase-test-target.mjs`, `apps/desktop/tests/integration/support/deterministic-services.mjs`, `apps/desktop/tests/fixtures/golden-path/**`, `docs/features/fix/13-test-foundation-phase1a.md`, `docs/harness/fixtures/README.md`, `docs/harness/fixtures/golden-path/README.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` |
| 2026-05-22 | Codex | Accepted Claude's Phase 1A review corrections: changed the embedding fixture contract from 384 to 2048 dimensions, accepted ADR 0006 after review, defined schema provenance through full repository migration-stream setup in a disposable Supabase test target, added dev DB refusal guardrails, and narrowed Phase 1B's first golden-path contract to the core paper-to-table persistence spine without starting runtime implementation | `docs/features/fix/13-test-foundation-phase1a.md`, `docs/harness/decisions/0006-test-fixture-strategy.md`, `docs/harness/fixtures/golden-path/README.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` |
| 2026-05-22 | Codex | Drafted Test Foundation Phase 1A after Plan 12 closure: added the fixture strategy and harness skeleton plan, proposed ADR 0006 with isolated local Supabase as the primary integration fixture and pglite deferred to optional pure-helper coverage, documented the golden-path fixture corpus contract, and requested Claude blockers/P1/P2 go-stop review before Phase 1B runtime test implementation | `docs/features/fix/13-test-foundation-phase1a.md`, `docs/harness/decisions/0006-test-fixture-strategy.md`, `docs/harness/fixtures/README.md`, `docs/harness/fixtures/golden-path/README.md`, `docs/agents/codex-claude/codex-to-claude.md`, `AGENTS.md` |
